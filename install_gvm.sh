#!/usr/bin/env bash
######################################################################
# Ubuntu20.Greenbone Vulnerability Management / OpenVASをインストールするためのスクリプト.
#
# Note: rootとして実行.
#
...
if [[ -z "${GVMVERSION}" ]]
then
  printf "インストールするにはバージョン番号を入力する必要があります。\n"
  print_help
fi

if [[ $GVMVERSION = "21" ]] || [[ $GVMVERSION = "20" ]]; then
    echo "バージョン$GVMVERSIONをインストールします。"
    setup_init
else 
    echo "入力$GVMVERSIONを理解できませんでした。"
...
# OpenVASおよびOpenVAS SMBをビルドしてインストールします。
...
# OSPd と OSPd-OpenVAS の構築とインストール
...
# Start GVM
# Greenbone Vulnerability Managementを起動する.
...
echo "訳: ユーザー名はgvmadmin、パスワードはStrongPass."

echo "Remember to change this default password"
echo "訳: このデフォルトのパスワードを変更することを忘れないでください."
